...
module Make = (Config: Config) => {
  ...
  let apolloDataToReason: renderPropObjJS => response = apolloData =>
    switch (
      apolloData.loading,
  ...
  let make = (~variables=?, ~onError=?, ~onCompleted=?, ~children) =>
    <JsMutation mutation=graphqlMutationAST ?variables ?onError ?onCompleted>
      {(mutation, apolloData) =>
        children(apolloMutationFactory(~jsMutation=mutation), apolloData |> convertJsInputToReason)}
    </JsMutation>
}
