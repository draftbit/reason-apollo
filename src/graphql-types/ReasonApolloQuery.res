...
module Make = (Config: Config) => {
  @module("graphql-tag") external gql: gql = "default"
  ...
  let convertJsInputToReason = (apolloData: renderPropObjJS) => {
    ...
    error: switch apolloData.error |> Js.Nullable.toOption {
    | Some(error) => Some(error)
    | None => None
    },
    loading: apolloData.loading,
    refetch: variables =>
      apolloData.refetch(variables |> Js.Nullable.fromOption) |> Js.Promise.then_(data =>
        data |> apolloDataToVariant |> Js.Promise.resolve
      ),
    fetchMore: (~variables, ~updateQuery, ()) =>
      apolloData.fetchMore({variables: variables, updateQuery: updateQuery}),
